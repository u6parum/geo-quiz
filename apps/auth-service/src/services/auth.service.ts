...
export const authService = {
  ...
  async refresh(refreshToken: string) {
    ...
    if (!stored) throw new Error('Refresh-токен не найден');
    if (stored.revokedAt) throw new Error('Refresh-токен отозван');
    if (stored.expiresAt < new Date()) throw new Error('Refresh-токен истёк');
